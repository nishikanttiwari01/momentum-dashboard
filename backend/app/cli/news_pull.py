...
def _load_watchlist_symbols(path: Optional[str]) -> List[str]:
    ...
    for line in p.read_text(encoding="utf-8").splitlines():
        ...
        if not s or s.startswith("#"):
            continue
        symbols.append(s)
    return _unique_preserve(symbols)
...
def _select_from_scores_latest(
    *,
    ...
    # Read without filters first to rank movers
    ...
    if score_min is not None:
        # pick proper score column preference
        score_field = "score_full" if score_type == "full" else "score_basic"
        # Build filter: score_field >= score_min (fallback to generic 'score' if needed)
...
def run_intraday(
    *,
    ...
    if not symbols:
        log.info("news.run_intraday_no_symbols")
        _stderr("[info] no symbols selected for intraday run; exiting")
        return
    log.info("news.run_intraday_execute", extra={"symbols": len(symbols)})
    _stderr(
    ...
    log.info("news.run_intraday_provider_done", extra={"items": len(batch.items)})
    _ingest_batch_via_api(api_base, batch)
    log.info("news.run_intraday_ingested", extra={"items": len(batch.items)})
...
def run_backfill(
    *,
    ...
    log.info(
        ...
        extra={"trading_day": trading_day.isoformat(), "symbol_count": len(symbols)},
    )
    if not symbols:
        _stderr("[info] no symbols found for backfill; exiting")
        log.info("news.run_backfill_no_symbols", extra={"trading_day": trading_day.isoformat()})
        return
    ...
    if provider_cmd_clean:
        log.info(
            ...
            extra={"trading_day": trading_day.isoformat(), "provider_cmd": provider_cmd_clean},
        )
        batch = _run_provider_cmd(
        ...
        log.info(
            ...
            extra={"trading_day": trading_day.isoformat(), "provider_cmd": provider_cmd_clean, "item_count": len(batch.items)},
        )
    else:
        log.info(
            "news.run_backfill_provider_internal",
            extra={"trading_day": trading_day.isoformat(), "since_minutes": since_minutes, "anchor": at_iso},
        )
        batch = _build_batch(symbols=symbols, since_minutes=since_minutes, anchor=anchor_dt)
        log.info(
            "news.run_backfill_provider_internal_done",
            extra={"trading_day": trading_day.isoformat(), "item_count": len(batch.items)},
        )
