  ...
  return (
    <Box data-testid="fund-nav-chart" sx={{ py: 1.5, px: { xs: 0.5, md: 1 } }}>
      <Stack direction="row" alignItems="center" spacing={2} flexWrap="wrap" useFlexGap sx={{ mb: 1 }}>
        ...
        {data?.latest_vs_average_pct != null ? <Typography variant="caption" sx={{ color: data.latest_vs_average_pct <= 0 ? '#079455' : '#C2410C', fontWeight: 600 }}>Latest NAV is {Math.abs(data.latest_vs_average_pct).toFixed(2)}% {data.latest_vs_average_pct < 0 ? 'below' : 'above'} average</Typography> : null}
        <Box sx={{ flexGrow: 1 }} />
        <ToggleButtonGroup
          size="small"
          exclusive
      ...
      {query.isLoading ? (
        ...
        <Box sx={{ width: '100%', height: 300, bgcolor: '#FFFFFF', px: 1, pt: 1.5, overflow: 'hidden' }}>
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 18, right: 28, bottom: 4, left: 0 }}>
              <CartesianGrid stroke="#F5F5F5" vertical={false} />
              <XAxis dataKey="time" type="number" scale="time" domain={timeDomain} allowDataOverflow tick={{ fontSize: 10, fill: '#9b9b9b' }} axisLine={{ stroke: '#ECECEC' }} tickLine={false} minTickGap={40} tickMargin={8} tickFormatter={(value: number) => dayjs(value).format(labelFmt)} />
              <YAxis
              ...
              <ReTooltip
                ...
                contentStyle={{ borderRadius: 6, borderColor: '#ECECEC', fontSize: 12 }}
              />
              <Line type="monotone" dataKey="nav" stroke="#2E90FA" strokeWidth={1.8} dot={false} activeDot={{ r: 5, fill: '#2E90FA', stroke: '#fff', strokeWidth: 2 }} isAnimationActive={false} />
              ...
              {data?.average_nav != null ? <ReferenceLine y={data.average_nav} stroke="#B0B4BE" strokeDasharray="5 4" label={{ value: `Average NAV ${data.average_nav.toFixed(2)}`, position: 'insideTopRight', fill: '#9b9b9b', fontSize: 10 }} /> : null}
              {latestPoint ? <ReferenceDot x={latestPoint.time} y={latestPoint.nav} r={4} fill="#2E90FA" stroke="#FFFFFF" strokeWidth={2} ifOverflow="extendDomain" label={{ value: `Latest NAV ${latestPoint.nav.toFixed(2)}`, position: 'top', dx: -52, fill: '#667085', fontWeight: 600, fontSize: 10 }} /> : null}
            </ComposedChart>
          </ResponsiveContainer>
        </Box>
      ) : (
