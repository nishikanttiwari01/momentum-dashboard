...
function wealthMoney(value: number | null): string {
  ...
  if (Math.abs(value) >= 10_000_000) return `â‚¹${(value / 10_000_000).toFixed(2)} Cr`;
...
export const PortfolioSummaryHeaderView: React.FC<{ summary: WealthSummary }> = ({ summary }) => {
  ...
  return <Paper variant="outlined" sx={{ p: 2, borderRadius: 3, borderColor: '#DCE5F0', boxShadow: '0 12px 32px rgba(22,34,58,0.06)' }}>
    <Stack direction={{ xs: 'column', md: 'row' }} spacing={2.5} alignItems={{ md: 'center' }}>
      ...
      <Box sx={{ flexGrow: 1 }} />
      {summary.fx ? <Chip icon={<CurrencyExchangeRoundedIcon />} variant="outlined" color={summary.fx.is_fallback ? 'warning' : 'success'} label={`USD/INR ${summary.fx.rate.toFixed(2)} Â· ${summary.fx.effective_on}${summary.fx.is_fallback ? ' cached' : ''}`} /> : null}
      <Chip icon={<AccountBalanceWalletRoundedIcon />} label={`As of ${summary.as_of ?? 'â€”'}`} />
    </Stack>
  </Paper>;
};
