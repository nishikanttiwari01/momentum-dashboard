...
export const PassiveIncomePanel: React.FC<{ analysis: PassiveIncomeAnalysis }> = ({ analysis }) => <Box component="section" aria-labelledby="passive-income-title" sx={{ bgcolor: '#F5F9FE', border: '1px solid #CFE0F2', borderLeft: `5px solid ${analysis.on_track ? '#0F9D8A' : '#DC4C4C'}`, borderRadius: 3, p: { xs: 2, md: 3 } }}>
  <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap' }}><Box><Typography id="passive-income-title" variant="h6" fontWeight={800} color="#102A43">2029 income runway</Typography><Typography variant="body2">Rent counts toward the ₹2 lakh target</Typography></Box><Chip label={analysis.on_track ? 'On track' : 'Shortfall'} color={analysis.on_track ? 'success' : 'error'} /></Box>
  ...
  <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, borderTop: '1px solid #CFE0F2', pt: 1.5, flexWrap: 'wrap' }}><Box><Typography variant="caption">Surplus / shortfall</Typography><Typography fontWeight={800} color={analysis.surplus_or_shortfall_inr >= 0 ? '#137333' : '#B3261E'}>{signedMonthly(analysis.surplus_or_shortfall_inr)}</Typography></Box><Box><Typography component="span" variant="caption">Earliest sustainable date</Typography><strong style={{ display: 'block' }}>{dateLabel(analysis.earliest_sustainable_date)}</strong></Box></Box>
